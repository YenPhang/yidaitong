...
$(function() {
	...
	$(".input_three").keyup(function() {
		var text0 = val = $(this).val().replace(/,/g, ""),
			arr = [],
			text1 = "";
		if(val.indexOf(".") > -1) {
			arr = val.split(".");
		...
		if(document.cookie && document.cookie != '') {
			...
			for(var i = 0; i < cookies.length; i++) {
				...
				if(cookie.substring(0, name.length + 1) == (name + '=')) {
					var pattern = /["]+/gi,
						names = cookie.substring(name.length + 1);
					cookieValue = decodeURIComponent(names.replace(pattern, ""));
					break
				}
			}
		};
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.success || data.length == 0) {
					if(flag == 2) {
						$obj2.attr("disabled", "disabled").val("120").css({
						...
						timer = setInterval(function() {
							if(i == 0) {
								clearInterval(timer);
								$obj2.val("重新发送").removeAttr("disabled").css({
...
function registerFenliu() {
	...
	if(Terminal.platform.android) {
		$("#fenliu,#fenliu1").attr("href", "https://app.etongdai.com/register/wapRegister");
		// alert('小屏');
...
function getIndexHtml(Stringserver) {
	...
	if(userName) {
		loginHtml = "<div class='welcome'>欢迎您，</div>" +
			"<a href=" + Stringserver.sslServer + "/account/index>" + userName + "</a><a class='exit' href=" +
			Stringserver.server + "/login/logout>退出登录</a>";

		indexHtml = "<a class='account' href=" + Stringserver.sslServer + "/account/index>" +
			"<i class='iconfont'>&#xe637;</i>" + "我的账户</a>"

	} else {

		indexHtml = "<span id='register_log'>" +
			"<a id='fenliu' href=" + Stringserver.sslServer + "/register/index>注册</a>" + "<i>/<i>" + "<a id='fenliu_log' href=" + Stringserver.sslServer + "/login/index>登录</a>" +
			"</span>"

	}
	$("#accountStatus").append(indexHtml);
