...
$(function () {
    ...
    if(canInvestCash <= minInvestMoney){
        $(".jian").addClass("but_in");$(".jia").addClass("but_in");$(".jia").addClass("but_in"); $(".jian").unbind("click");
    ...
    if($("#money").val()>0){
        ...
        if(nowMoney == minInvestMoney && nowMoney == canInvestCash){ //如果投资金额等于最低投资金额并且投资金额等于可投金额  加减号置灰
            $(".jian").addClass("but_in");$(".jia").addClass("but_in");$(".jia").addClass("but_in"); $(".jian").unbind("click");
            $('.btn_m_all').removeAttr('onclick');$(".btn_m_all").addClass("but_in");
        }else if(nowMoney == minInvestMoney){
            minusFlag=1;
            $(".jian").addClass("but_in"); $(".jian").unbind("click");
        }else if(nowMoney == canInvestCash){
            addFlag=1;
            $(".jia").addClass("but_in"); $(".jia").unbind("click");
        }
        computeFun_tz();
    }
    ...
    function minusMoney(){
        ...
        if(totalnum<=minInvestMoney){ //如果当前金额 小于等于 最低投资金额  显示最低投资金额  减号置灰
            ...
            $(".jian").unbind("click");$(".jian").addClass("but_in");
            if(totalnum < canInvestCash){
    ...
    $(".btn_m_all").click(function() {
        ...
        if(nowMoney == canInvestCash){
            ...
            minusFlag=0;
            $(".jia").unbind("click");$(".jia").addClass("but_in");
            addFlag=1;
        }
   ...
   function moneyEmit(){
    ...
    if (swapLiLength == 5 || swapLiLength > 5) {
        $(".swiperBox>ul>li").css({"margin-right": "15px"});
    }
    var swiperDivWidth = $(".swiperDiv").width();
    $(".aptitude").css({"width": swiperDivWidth + 80});
    //未实名认证层隐藏
    ...
    $('.tab_t a').click(function() {
        ...
        if(css.indexOf("query")==-1){
            ...
            var $iteId=$("#iteId_val"),$userId=$("#userId_val");
...
function transfer(){
    ...
    $.ajax({
       ...
       data: {"claTransId":$("#claTransId_val").val(),"type":"complete","password":$("#password").val(),"money":$("#money").val()},
       async: false,
       success: function (data) {
           data = $.parseJSON(data);
           if (data.success) {
...
function computeFun_tz() {
    ...
    var repayment = 0, earnMoneyNPrize;
...
function scrollLis() {
    ...
    if (toTop == 0 || toTop < 0) {
        if (!$('#fixed').hasClass('ab')) {
            $('#fixed').addClass('ab');
            $(".fixed_bd").css({"padding-top":"65px"})
        }
    } else {
        $('#fixed').removeClass('ab');
        $(".fixed_bd").css({"padding-top":"10px"})
    }
}
